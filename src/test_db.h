...
void TestsMyCustom()
{
...

    {
        ...
        auto l = db.Last({ 2019, 12, 4 });
        Assert((l.first == Date{2019, 12, 3}) && (l.second == "f"),  " My test 6");
        ...
        l = db.Last({ 2019, 12, 2 });
        Assert((l.first == Date{ 2019, 12, 2 }) && (l.second == "c c"), " My test 7");

        l = db.Last({ 2019, 12, 4 });
        Assert((l.first == Date{ 2019, 12, 3 }) && (l.second == "f"), "My test 8");
        
        db.Add({ 2019, 12, 3 }, "m");

        l = db.Last({ 2019, 12, 3 });
        Assert((l.first == Date{ 2019, 12, 3 }) && (l.second == "m"), "My test 9");
...

    {
        ...
        auto l = db.Last({ 2019, 12, 1 });
        Assert((l.first == Date{ 2019, 12, 1 }) && (l.second == "c"), " My test 20");

        db.Add({2019, 12, 1}, "b");

        l = db.Last({ 2019, 12, 1 });
        Assert((l.first == Date{ 2019, 12, 1 }) && (l.second != "c"), "My test 21");
...
void TestDatabase() {
  ...
  // Last
  {
    ...
    auto l = db.Last(d);
    Assert((l.first == Date{ 2018, 12, 22 }) && (l.second == "e2"), "Db Last 1");

    Date d3(2018, 12, 24);
    db.Add(d3, "e3");

    l = db.Last(d);
    Assert((l.first == Date{ 2018, 12, 24 }) && (l.second == "e3"), "Db Last 2");
    ...
    l = db.Last(d);
    Assert((l.first == Date{ 2018, 12, 22 }) && (l.second == "e2"), "Db Last 4");

    l = db.Last(d1);
    Assert((l.first == Date{ 2019, 1, 2 }) && (l.second == "e1"), "Db Last 5");

    db.Add(d2, "e4");

    l = db.Last(d2);
    Assert((l.first == Date{ 2018, 12, 22 }) && (l.second == "e4"), "Db Last 6");
  }
